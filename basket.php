...
<head>
  ...
  <title>AuThOrIzKeY - Желаемое</title>
</head>
...
<header class="header">
  ...
  <main class="main">
    ...
    if(!isset($_SESSION["id"])) {
        ...
        <div class="cart_box">
            <h3 class="product">
              Чтобы получить доступ к списку желаемого нужно <a href="log_in.html">авторизироваться</a>!
            </h3>
        </div>
        ';
    }
      ...
      $bsktitem = $connect->getBasketItem($productall);
      foreach ($bsktitem as $cartbtn) {
      echo '
      <div class="cart_box">
        <h3 class="product">
        ', $cartbtn['name'],' - ', $cartbtn['price'],' баллов
        </h3>
        <a href="../php/basket_delete.php?c=payments&cartbtn=', $cartbtn['id'],'" class="del_btn">Удалить</a>
        <a href="../php/basket_to_cart.php?c=payments&cartbtn=', $cartbtn['id'],'" class="buy_btn">Переместить в корзину</a>
      </div>
        ';
      }}?>
